...
from utils.transform import GaussianBlur, ResizeToResolution, Identity, RandomResize
...
from models.resnet import deeplab_resnet 
...
parser.add_argument('--feature_frac', default=64, type=int)
...
MASK_SIZE = math.ceil(PRETEXT_RESOLUTION/args.feature_frac)
...
with open(TRAIN_CSV) as f:
    ...
    train_dict = {}
    valid_dict = {}
    train_dict['ImagePath'] = [name.strip() + '.jpg' for name in names]
    train_dict['index'] = [0 for idx in names]
    valid_dict['ImagePath'] = [name.strip() + '.jpg' for name in names]
    valid_dict['index'] = [0 for idx in names]
    df_train = pd.DataFrame(train_dict)
    df_valid = pd.DataFrame(valid_dict)
...
data_transforms = {
    'preprocess_pretext': transforms.Compose([
        ...
        transforms.Resize((int(PRETEXT_RESOLUTION/5*6),int(PRETEXT_RESOLUTION/5*6))),
        #transforms.Resize((PRETEXT_RESOLUTION,PRETEXT_RESOLUTION))
        #transforms.Resize((int(PRETEXT_RESOLUTION/7*8),int(PRETEXT_RESOLUTION/7*8))),
        #RandomResize(5/4,PRETEXT_RESOLUTION),
    ]),
    'preprocess_train': transforms.Compose([
        transforms.Resize((int(DOWNSTREAM_RESOLUTION/5*6),int(DOWNSTREAM_RESOLUTION/5*6))),
        #transforms.Resize((DOWNSTREAM_RESOLUTION,DOWNSTREAM_RESOLUTION))
        #transforms.Resize((int(PRETEXT_RESOLUTION/7*8),int(PRETEXT_RESOLUTION/7*8))),
        #RandomResize(5/4,DOWNSTREAM_RESOLUTION),
    ]),
    ...
    'totensor_valid': transforms.Compose([
        #transforms.Resize((DOWNSTREAM_RESOLUTION,DOWNSTREAM_RESOLUTION)),
        transforms.CenterCrop((DOWNSTREAM_RESOLUTION,DOWNSTREAM_RESOLUTION)),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
...
data_set = {
    ...
    'valid': dataset(img_dir=VALID_DIR, anno_pd=df_valid, preprocess=Identity(),
                      augment=Identity(), totensor=data_transforms["totensor_valid"],
...
dataloader = {
    'pretext': torch.utils.data.DataLoader(data_set['pretext'], batch_size=PRETEXT_BATCH_SIZE, shuffle=True, 
                                           num_workers=NUM_WORKER, collate_fn=collate_fn_pretext, drop_last=True),
    'train': torch.utils.data.DataLoader(data_set['train'], batch_size=DOWNSTREAM_BATCH_SIZE, shuffle=True, 
                                         num_workers=NUM_WORKER, collate_fn=collate_fn_train, drop_last=True),
    'valid': torch.utils.data.DataLoader(data_set['valid'], batch_size=DOWNSTREAM_BATCH_SIZE, shuffle=False, 
                                         num_workers=NUM_WORKER, collate_fn=collate_fn_valid, drop_last=True)
}
...
if MODEL_NAME == "resnet":
    ...
    model = deeplab_resnet(stage=STAGE, in_dim=2048, out_dim=SIMCLR_OUT_DIM, num_classes=NUM_CLASSES, size=MASK_SIZE, 
                   with_LIO=WITH_LIO, attention=ATTENTION, num_positive = NUM_POSITIVE, M = M, 
                   pretext = True, pretrain = PRETEXT_PRETRAIN_IMAGENET, deeper = DEEPER, config = config)
